...
- **Code documentation clarity (LOCKED, C-22)**: Every script Claude writes ships with (a) a short module docstring (2–4 sentences, plain-language WHAT + WHY), (b) a sibling `<script>.md` ≤100 lines deliberately simpler than the existing four sidecars (no frontmatter, no schema dumps, no nested tables — sections: plain-language summary · how to call · in/out · where it fits · gotchas-if-real). Target audience: a non-CS reader can describe the script in their own words after one read.
