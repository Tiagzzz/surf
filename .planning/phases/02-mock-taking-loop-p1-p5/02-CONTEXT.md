...
- **D-2.25a (Component validation flow — NEW 2026-05-03, Tiago ruling during Plan 02-01 Task 11):** Bench approval is **not** production approval. Every component in `ui/documentation.md § 5` carries a `Status` flag with four valid values: `bench-v1` (CSS exists + bench-validated; not yet seen in real page context), `production-locked` (validated inside a downstream plan's preview gate where the component appears in its real composition), `draft` (CSS exists but never bench-validated), `deferred` (spec'd or surfaced but not yet built). The flip from `bench-v1` to `production-locked` happens **per-plan, per-component**, earned by the plan's preview gate — never retroactively from bench approval alone. § 1.1 of `ui/documentation.md` explains the flow to readers; § 5 carries the per-component Status. **Edit-this-later note:** when a plan ships a real-context preview gate, the plan's executor updates the corresponding § 5 row's Status to `production-locked` and records the plan ID + commit SHA in the "Earned where" column.
